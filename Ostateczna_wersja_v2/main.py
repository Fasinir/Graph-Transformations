...
def PerformProduction(SavePng):
    ...
    if ChosenProduction != -1 and len(ProductionList) != 0:
        ...
        if performed:
            ...
            LogListBox.insert(END, current_time + " Produkcja #" + str(
                ChosenProduction) + " nie znaleziono wierzchołka lewej strony produkcji")
...
def RandomProdFunc():
    ...
    SaveEveryCheckbox = Checkbutton(popup, text="Zapisać każdą produkcję?", onvalue=True, offvalue=False,
                                    variable=SaveEveryVar)
...
def LoadProdFunc():
    ...
    def browseFiles():
        ...
        if type(filename) is str:
            label_file_explorer.configure(text="File Opened: " + filename)
            parseProductions(ProductionList, filename)
...
LoadProductionButton = Button(LeftFrame, text="Wczytaj produkcje", height=2, width=20, command=LoadProdFunc, padx=5,
                              pady=5)
LoadProductionButton.pack()

ShowHideChangesButton = Button(LeftFrame, text="Pokaż/Ukryj zmiany", height=2, width=20, command=ShowHideFunc, padx=5,
                               pady=5)
ShowHideChangesButton.pack()

DoProductionButton = Button(LeftFrame, text="Wykonaj produkcję", height=2, width=20,
                            command=lambda: PerformProduction(True), padx=5, pady=5)
DoProductionButton.pack()

DoRandomProductionsButton = Button(LeftFrame, text="Wykonaj losowe\n produkcje", height=4, width=20,
                                   command=RandomProdFunc, padx=5, pady=5)
DoRandomProductionsButton.pack()

SequenceOfProductionsButton = Button(LeftFrame, text="Wykonaj sekwencje\n produkcji", height=4, width=20,
                                     command=SequenceFunc, padx=5, pady=5)
...
DefineStatisticsLabelsButton = Button(LeftFrame, text="Zdefiniuj wierzchołki\n do statystyk",
                                      command=DefineStatisticsListFunc, height=4, width=20, padx=5, pady=5)
DefineStatisticsLabelsButton.pack()

ChosenNumberLabel = Label(text="Nie wybrano\n produkcji")
ChosenNumberLabel.place(relx=0, rely=0.75)

UpProdButton = Button(LeftFrame, text="/ \\\n|", command=UpProdFunc)
UpProdButton.place(relx=0.7, rely=0.70)

DownProdButton = Button(LeftFrame, text="|\n\\ /", command=DownProdFunc)
# DownProdButton.pack()
DownProdButton.place(relx=0.7, rely=0.75)

ExportGraphButton = Button(LeftFrame, text="Eksportuj graf", command=ExportGraphFunc, height=2, width=20, padx=5,
                           pady=5)
...
ProductionListbox.place(relwidth=1, relheight=0.2, rely=0.8)
